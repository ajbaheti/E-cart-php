			...
			try{				
				...
				if($row->getProdQuantity() > 0){	//if quantity is greater than 0
					...
					if($rowdata){
						...
						$stmt3->bindParam("ProdId",$row->getProdId(),PDO::PARAM_INT);
			...
			try{				
				...
				if(count($data) > 0){
					foreach($data as $row){
						...
						$stmt1 = $this->dbh->prepare("update products set ProdQuantity = :quant where ProdId = :pid");
						$stmt1->bindParam("pid",$row->getProdId(),PDO::PARAM_INT);
						$stmt1->bindParam("quant",$row->getProdQuantity(),PDO::PARAM_INT);
						$stmt1->execute();
